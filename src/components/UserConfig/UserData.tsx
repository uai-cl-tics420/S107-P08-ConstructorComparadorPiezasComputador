...
export function UserData() {
  ...
  const [passwordError, setPasswordError] = useState<Record<string, string | null>>({
    newPass: null, confirmPass: null, currentPass: null,
  });
  ...
  useEffect(() => {
    if (!pendingChanges.name) { setName(''); setNameModified(false); }
        ...
        <div className='flex flex-col gap-4 px-3 mt-2'>
          ...
          <div className='flex flex-row flex-wrap gap-6'>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value.length > 0) {
                    ...
                    setPendingChanges!((prev) => { const { name, ...rest } = prev; return rest; });
                  }
                }}
              />
              {nameModified && <motion.div initial={{ scaleX: 1 }} className='mt-0.5 h-0.5 w-46 origin-left rounded-full bg-tw-success' />}
            </div>
            ...
            <div className='flex flex-col'>
              <label className='text-tw-muted uppercase tracking-wider'>
                {passwordConfigured ? t('userData.changePassword') : t('userData.setPassword')}
              </label>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!e.target.value.length) {
                        ...
                        setPasswordData({ password: '', confirmPassword: '', currentPassword: '' });
                        setPasswordError({ newPass: null, confirmPass: null, currentPass: null });
                        setPendingChanges!((prev) => { const { password, currentPassword, confirmPassword, ...rest } = prev; return rest; });
                        return;
                      }
                      ...
                      if (!passwordData.currentPassword.length && passwordConfigured) {
                        setPasswordError((prev) => ({ ...prev, currentPass: t('userData.errorEnterCurrentPassword') }));
                      }
                      ...
                      <div className='flex flex-col ml-1 pl-3 mt-1 pt-1 border-l border-tw-muted-deep'>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (!e.target.value.length) {
                              setPasswordError((prev) => ({ ...prev, confirmPass: t('userData.errorRewritePassword') }));
                              setPendingChanges!((prev) => { const { confirmPassword, ...rest } = prev; return rest; });
                              return;
                            }
                        ...
                        {passwordConfigured && (
                          <>
                            <span className='text-tw-muted text-xs uppercase mt-2'>{t('userData.currentPassword')}</span>
                            <input
                              type='password'
                              className={`bg-tw-surface border rounded w-46 px-3 py-1 transition-all focus:outline-none ${
                              ...
                              onChange={(e) => {
                                ...
                                if (!e.target.value.length) {
                                  setPasswordError((prev) => ({ ...prev, currentPass: t('userData.errorEnterCurrentPassword') }));
                                  setPendingChanges!((prev) => { const { currentPassword, ...rest } = prev; return rest; });
                                  return;
                                }
                                setPendingChanges!((prev) => ({ ...prev, currentPassword: e.target.value }));
                                const validation = validatePassword(e.target.value);
                                if (!validation.isValid) {
                                  setPasswordError((prev) => ({ ...prev, currentPass: t('userData.errorInvalidCurrentPassword') }));
                                } else {
                ...
                <div>
                  {Object.values(passwordError).some((e) => e !== null) && (
                    <motion.div
                      ...
                      {Object.values(passwordError).filter((e) => e !== null).join('\n\n')}
                    </motion.div>
                  )}
                </div>
              </div>
