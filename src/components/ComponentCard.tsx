...
export function ComponentCard({ component, onAdd, onCompare, onViewSpecs, isSelectedForCompare, buildCompatibility, willReplace }: Props) {
  ...
  // Solo se muestran los 2 mejores precios; el resto se ve en el modal de detalle
  // (al hacer click en la tarjeta), en vez de expandir la casilla in situ.
  const visiblePrices = component.prices.slice(0, 2);
  ...
  return (
    <motion.div
      ...
      <div
        ...
        role={onViewSpecs ? 'button' : undefined}
        className={`relative h-full rounded-xl border bg-tw-surface-deep hover:bg-tw-surface backdrop-blur-xl p-5 z-10 flex flex-col gap-4 transition-[border-color] duration-300 ${onViewSpecs ? 'cursor-pointer' : ''} ${
        ...
        <div className='mt-auto space-y-3'>
          ...
          <div className='bg-tw-base border border-tw-border-deep rounded-lg p-2.5 space-y-1.5'>
            ...
            {hasMorePrices && (
              <motion.button
                onClick={(e) => { e.stopPropagation(); onViewSpecs?.(component); }}
                ...
                <span className='font-mono text-[0.6rem] uppercase tracking-[0.15em]'>
                  {t('card.seeMore', { count: hiddenCount })}
                </span>
        ...
        <div className='flex gap-2 pt-3 border-t border-tw-border-deep'>
          <motion.button
            ...
            {willReplace ? <ArrowLeftRight className='w-3.5 h-3.5' /> : <Plus className='w-3.5 h-3.5' />}
            {willReplace ? t('card.replace') : t('card.add')}
          </motion.button>
          {onViewSpecs && (
            <motion.button
