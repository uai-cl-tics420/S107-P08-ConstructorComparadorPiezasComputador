...
import { type Component, type ComponentMirror } from '@/types/Database_types';

// Interface types for component creation
...
interface GetComponentIDResponse {
  success: boolean;
  component_id?: string;
  error?: string;
}
...
export async function getComponentID(mongoDb: Db, name_model: string): Promise<GetComponentIDResponse> {
  try {
