...
export function SavedBuildsPanel({ savedBuilds, onLoad, onDelete }: Props) {
  return (
    <div className='bg-tw-base-highlight/60 border border-tw-glass/5 rounded-2xl p-4 flex flex-col gap-3'>
      <h3 className='text-tw-primary text-sm font-semibold'>Builds guardados</h3>

      {savedBuilds.length === 0 ? (
        <div className='flex flex-col items-center justify-center py-8 gap-2'>
          <span className='text-3xl'>💾</span>
          <p className='text-tw-muted text-sm text-center'>No hay builds guardados aun</p>
          <p className='text-tw-muted-deep text-xs text-center'>Guarda tu build actual para acceder despues</p>
        </div>
      ) : (
            ...
            <div
              ...
              <div className='flex items-start justify-between gap-2'>
                <div className='flex flex-col gap-0.5 min-w-0'>
                  ...
                  <p className='text-tw-muted text-xs'>{formatDate(build.created_at)}</p>
                </div>
