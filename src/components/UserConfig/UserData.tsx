  ...
  useEffect(() => {
    ...
    if (!pendingChanges.password) {
      ...
      const checkPassword = async () => {
        ...
        const hasPasswordConfigured = accounts?.some((account) => account.providerId === 'credential');
          ...
          <div className='flex flex-row flex-wrap gap-6'>
            ...
            <div className='flex flex-col'>
              <label className=' text-tw-muted uppercase tracking-wider'>
                {passwordConfigured ? 'Cambiar contraseña:' : 'Configurar contraseña'}
              </label>
              <div className='flex flex-row gap-6'>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!(passwordData.confirmPassword.length > 0)) {
                        setPasswordError((prev) => ({ ...prev, confirmPass: 'Reescriba la contraseña' }));
                      } else {
                        if (e.target.value !== passwordData.confirmPassword) {
                          setPasswordError((prev) => ({ ...prev, confirmPass: 'Las contraseñas no coinciden' }));
                        } else {
                          setPasswordError((prev) => ({ ...prev, confirmPass: null }));
                        }
                      }

                      if (!(passwordData.currentPassword.length > 0) && passwordConfigured) {
                        setPasswordError((prev) => ({ ...prev, currentPass: 'Ingrese su contraseña actual' }));
                      }
                    ...
                    <>
                      <motion.div
                        ...
                        className={`mt-0.5 h-0.5 w-46 origin-left rounded-full ${
                          passwordError.newPass ? 'bg-tw-alert' : 'bg-tw-success'
                        }`}
                      />
                      <div className='flex flex-col ml-1 pl-3 mt-1 pt-1 border-l border-tw-muted-deep'>
                        ...
                        {passwordConfigured && (
                          <>
                            <span className='text-tw-muted text-xs uppercase mt-2'>Su contraseña actual:</span>
                            <input
                              type='password'
                              className={`bg-tw-surface border rounded w-46 px-3 py-1 transition-all focus:outline-none ${
                              ...
                              onChange={(e) => {
                                ...
                                if (!(e.target.value.length > 0)) {
                                  setPasswordError((prev) => ({
                                    ...prev,
                                    currentPass: 'Ingrese su contraseña actual',
                                  }));
                                  setPendingChanges!((prev) => {
                                ...
                                if (!validation.isValid) {
                                  setPasswordError((prev) => ({
                                    ...prev,
                                    currentPass: 'La contraseña actual no es válida',
                                  }));
                                  return;
                                }
                ...
                <div>
                  {Object.values(passwordError).some((error) => error !== null) && (
                    <motion.div
